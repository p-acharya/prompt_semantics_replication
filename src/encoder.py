...
parser.add_argument(
    "-m", "--model-name", type=str, default="albert-xxlarge-v2"
)
...
parser.add_argument(
    "-ns", "--num-shots", type=str, default="1,2,4,8,16,32,64,128,256"
)
...
parser.add_argument(
    "--subsample-subcase", type=int, default=100
)  # only for HANS diagnosis
...
parser.add_argument(
    "--production",
    action="store_true",
    help="suppress training metrics printing",
)
    ...
    def check_conflicting_targets(self, tokenizer: hf.AutoTokenizer) -> bool:
        ...
        for class_id, word in self.class_id_to_word.items():
            ...
            if token in possible_duplicates:
                raise ValueError(
                    f"Target word {word} as {token} has duplicates"
                )
...
class PromptDataCollator(hf.DataCollatorForLanguageModeling):
    def __call__(
        ...
        batch["labels"] = torch.where(
            ...
            torch.unsqueeze(target_word_ids, 1),
            -100,
        )

        if random.random() < 0.05:  # Print 5% of batches
            print("\n--- Batch Debug ---")
            for i in range(
                min(2, len(examples))
            ):  # Print first 2 examples of the batch
                print(
                    f"Example {i} Input: {self.tokenizer.decode(batch['input_ids'][i])}"
                )
                print(f"Example {i} Label ID: {examples[i]['label']}")
                print(f"Example {i} Target Word ID: {target_word_ids[i]}")
                print(f"Example {i} Labels Tensor: {batch['labels'][i]}")
...
class PromptTrainer(hf.Trainer):
    ...
    def evaluation_loop(
        ...
        total = 0
        for batch in tqdm(
            dev_dataloader, desc="Evaluating Dev", disable=not args.debug
        ):
            ...
            if args.non_rank_eval:
                ...
                )  # only need logits of the [mask] token
                logits = logits.view(
                    batch_size, -1
                )  # batch_size * max_seq_len
                class_logits = logits[
                    :, self.label_word_ids
                ]  # (ent_id, neu_id, cont_id)
                pred_class = class_logits.argmax(
                    dim=-1
                )  # choose class with largest logits
        ...
        return hf.trainer_utils.EvalLoopOutput(
            ...
            label_ids=None,
            metrics=metrics,
            num_samples=len(dev_dataloader),
        )
...
def arrange_training(
    ...
    if len(train_set) <= args.train_batch_size:
        adjusted_train_batch_size = len(
            train_set
        )  # a batch has the entire train set
        eval_strategy = "epoch"
    ...
    trainer.label_word_ids = [
        wid for wid in prompt.class_id_to_word_id.values()
    ]
...
def main() -> None:
    if args.dataset in ("rte", "cb"):
        ...
        dev_set = hfd.load_dataset(
            "super_glue", args.dataset, split="validation"
        )
    ...
    out_path = (
        args.save_dir / f"{args.experiment_name}_s{args.current_seed}.csv"
    )
    ...
    with open(args.prompt_path) as p_file:
        ...
        for row in reader:
            ...
            if all(
                [arg_exp not in prompt_exp_names for arg_exp in arg_exp_names]
            ):
                continue
    ...
    for prompt in tqdm(prompts, desc="Prompts"):
        ...
        def prompt_and_tokenize(example: dict) -> dict:
            ...
            if random.random() < 0.01:  # Print 1% of the time to avoid spam
                print(f"DEBUG: Filled template: {filled_template}")
        ...
        for num_shots in tqdm(
            args.num_shots, desc="Num. Shots", disable=len(args.num_shots) == 1
        ):
...
if __name__ == "__main__":
    for seed in tqdm(
        args.seeds, desc="Random Seeds", disable=len(args.seeds) == 1
    ):
        hf.set_seed(seed)
